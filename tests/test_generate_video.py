...
def test_wan_adapter_is_vram_managed() -> None:
    assert WanAdapter.managed_vram is True


async def test_health_ok_when_model_not_loaded(tmp_path: Path) -> None:
    """Server reachable but model unloaded is healthy — the sequencer loads it later."""
    fake_httpx, mock_client = _mock_httpx()
...
async def test_unload_returns_false_when_service_down(tmp_path: Path) -> None:
    """Server unreachable ⇒ nothing resident ⇒ safe to continue rendering."""
...
async def test_unload_raises_on_http_error(tmp_path: Path) -> None:
    """Server up but refusing to unload (e.g. 409 load-in-progress) must abort loudly."""
    fake_httpx, mock_client = _mock_httpx()
...
async def test_wait_until_loaded_returns_when_ready(tmp_path: Path) -> None:
    fake_httpx, mock_client = _mock_httpx()
    responses = [
        {"status": "ok", "model_loaded": False, "loading": True, "error": None},
        {"status": "ok", "model_loaded": True, "loading": False, "error": None},
    ]
    mock_client.get.return_value.json = MagicMock(side_effect=responses)
    with patch.dict(sys.modules, {"httpx": fake_httpx}):
        await _adapter(tmp_path).wait_until_loaded(timeout_sec=60, poll_sec=0)
    assert mock_client.get.call_count == 2


async def test_wait_until_loaded_raises_on_load_error(tmp_path: Path) -> None:
    fake_httpx, mock_client = _mock_httpx()
    mock_client.get.return_value.json = MagicMock(
        return_value={"status": "ok", "model_loaded": False, "loading": False, "error": "CUDA OOM"}
    )
    with patch.dict(sys.modules, {"httpx": fake_httpx}):
...
async def test_wait_until_loaded_times_out(tmp_path: Path) -> None:
    fake_httpx, mock_client = _mock_httpx()
    mock_client.get.return_value.json = MagicMock(
        return_value={"status": "ok", "model_loaded": False, "loading": True, "error": None}
    )
    with patch.dict(sys.modules, {"httpx": fake_httpx}):
        with pytest.raises(TimeoutError, match="not loaded"):
