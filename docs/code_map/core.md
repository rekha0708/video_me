...
- `async _generate_shot_video(shot: Shot, script: Script, cast: Cast, adapters: _Adapters, work_dir: Path, image_uri: str, options: RunOptions | None=None, *, shot_index: int=1, total_shots: int=1) -> tuple[VideoClip, AudioTrack]` — Phase B: synthesize voice + generate video for one shot using the approved image.
