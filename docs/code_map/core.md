...
- `async _generate_shot_video(shot: Shot, script: Script, cast: Cast, adapters: _Adapters, work_dir: Path, image_uri: str, options: RunOptions | None=None) -> tuple[VideoClip, AudioTrack]` — Phase B: synthesize voice + generate video for one shot using the approved image.
