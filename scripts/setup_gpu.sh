...
if ! curl -sf http://localhost:7860/sdapi/v1/sd-models >/dev/null 2>&1; then
  cd "\$A1111_DIR"
  A1111_PTH="\/venv/lib/python3.10/site-packages/taming_transformers_repo.pth"
  if [[ -d "\/repositories/taming-transformers" && -d "\/venv/lib/python3.10/site-packages" ]]; then
